...
export async function bootApp(root = document) {
  ...
  const railRefs = {
    ...
    newNovel: root.querySelector("#new-novel"),
    projectCount: root.querySelector("#project-count"),
    projectFilter: root.querySelector("#project-filter"),
    projectList: root.querySelector("#project-list"),
    projectOpenStatus: root.querySelector("#project-open-status"),
    ...
    topbarMore: root.querySelector("#topbar-more"),
    topbarSecondary: root.querySelector("#topbar-secondary"),
    agentSurface: root.querySelector("#agent-surface"),
  ...
  const drawerRefs = {
    ...
    drawerTabs: root.querySelector("#drawer-tabs"),
  ...
  const readerRefs = {
    ...
    readerClose: root.querySelector("#reader-close"),
    readerBody: root.querySelector("#reader-body"),
    readerFontMinus: root.querySelector("#reader-font-minus"),
    readerFontPlus: root.querySelector("#reader-font-plus"),
    readerPrev: root.querySelector("#reader-prev"),
    readerNext: root.querySelector("#reader-next"),
  ...
  const settingsRefs = {
    ...
    settingsX: root.querySelector("#settings-x"),
    createScrim: root.querySelector("#create-scrim"),
    createHeading: root.querySelector("#create-heading"),
    createLead: root.querySelector("#create-card .lead"),
    createTitle: root.querySelector("#create-title"),
    createSeed: root.querySelector("#create-seed"),
    createChapters: root.querySelector("#create-chapters"),
    createMinWords: root.querySelector("#create-min-words"),
    createPath: root.querySelector("#create-path"),
    createBrowse: root.querySelector("#create-browse"),
    createSubmit: root.querySelector("#create-submit"),
    createStatus: root.querySelector("#create-status"),
    createX: root.querySelector("#create-x"),
    shortcutsScrim: root.querySelector("#shortcuts-scrim"),
    shortcutsX: root.querySelector("#shortcuts-x"),
  };
  ...
  topbarActions?.insertBefore(planPanel.chip, railRefs.topbarMore ?? topbarActions.firstChild);
  ...
  const sessionSidebar = createSessionSidebar({
    listEl: railRefs.projectList,
    countEl: railRefs.projectCount,
    filterEl: railRefs.projectFilter,
  ...
  function openRail() {
    ...
    railRefs.rail.classList.add("show");
    railRefs.railScrim.hidden = false;
    railRefs.railToggle.setAttribute("aria-expanded", "true");
  }
  
  function closeRail({ restoreFocus = true } = {}) {
    railRefs.rail.classList.remove("show");
    railRefs.railScrim.hidden = true;
    railRefs.railToggle.setAttribute("aria-expanded", "false");
    if (restoreFocus) railRefs.railToggle.focus();
  }
  ...
  function syncDrawerMode() {
    ...
    drawerRefs.drawer.setAttribute("role", modal ? "dialog" : "complementary");
    if (modal) drawerRefs.drawer.setAttribute("aria-modal", "true");
    else drawerRefs.drawer.removeAttribute("aria-modal");
    drawerRefs.drawerScrim.hidden = !modal || !drawerRefs.drawer.classList.contains("show");
  }
  
  const onViewportModeChange = () => {
    // 已打开的 drawer 因窗口变窄升级为模态：焦点移入抽屉，避免停留在背景。
    // media change 事件到达时 .matches 已是新值，旧态从 drawer 当前语义（role）判断。
    const openWide = drawerRefs.drawer.classList.contains("show") &&
      drawerRefs.drawer.getAttribute("role") === "complementary";
    syncDrawerMode();
    if (openWide && isDrawerModal()) drawerRefs.drawerClose.focus();
    // 窄屏 → 宽屏：收起 rail 覆盖态；drawer 不强制关闭（宽屏非模态可继续停留）。
    if (!railMedia.matches && railRefs.rail.classList.contains("show")) closeRail({ restoreFocus: false });
  };
  ...
  function openDrawer(tab) {
    ...
    setDrawerTabActive();
    drawerRefs.drawer.removeAttribute("inert");
    drawerRefs.drawer.classList.add("show");
    drawerRefs.drawer.setAttribute("aria-hidden", "false");
    ...
    if (isDrawerModal()) {
      ...
      drawerRefs.drawerScrim.classList.add("show");
      drawerRefs.drawerClose.focus();
    } else {
      // 宽屏非模态：scrim 保持隐藏，不抢正在输入 Composer 的焦点。
      drawerRefs.drawerScrim.classList.remove("show");
    }
    renderDrawerBody();
    ...
    void loadDashboard({ background: true });
    motion.openDrawer(drawerRefs.drawer, drawerRefs.drawerScrim, {
      body: drawerRefs.drawerBody,
      tabs: drawerRefs.drawerTabs
    });
  }
  
  function toggleDrawer() {
    if (drawerRefs.drawer.classList.contains("show")) closeDrawer();
    else openDrawer();
  }
  
  function openDrawerTab(tab) {
    if (drawerRefs.drawer.getAttribute('aria-hidden') !== 'false') toggleDrawer();
    setDrawerTab(tab);
  }
  
  function closeDrawer() {
    // 只有焦点当前位于 drawer 内，或模态打开时，才把焦点还给触发器。
    const restoreFocus = isDrawerModal() || drawerRefs.drawer.contains(document.activeElement);
    drawerRefs.drawer.dataset.closing = "true";
    drawerRefs.drawerScrim.dataset.closing = "true";
    motion.closeDrawer(drawerRefs.drawer, drawerRefs.drawerScrim, {
      onComplete: () => {
        delete drawerRefs.drawer.dataset.closing;
        delete drawerRefs.drawerScrim.dataset.closing;
        if (restoreFocus) {
          // lastFocused 是共享槽：drawer 内再开 settings/reader 会覆盖它。
          // 兜底：指向 drawer 内部或已断开时，退回 drawer 触发器。
          // ponytail: 多浮层焦点栈改造超出本任务范围；兜底保证焦点不丢失。
          const target = lastFocused && lastFocused.isConnected && !drawerRefs.drawer.contains(lastFocused)
            ? lastFocused
            : railRefs.openDrawer;
    ...
    drawerRefs.drawer.classList.remove("show");
    drawerRefs.drawer.setAttribute("aria-hidden", "true");
    drawerRefs.drawer.setAttribute("inert", "");
    drawerRefs.drawerScrim.classList.remove("show");
  }
  ...
  function setDrawerTabActive() {
    for (const button of drawerRefs.drawerTabs.querySelectorAll(".dtab")) {
  ...
  // 只切换现有 tab，不改变数据加载接口。
  drawerRefs.drawerTabs.addEventListener("keydown", (event) => {
    const tabs = [...drawerRefs.drawerTabs.querySelectorAll(".dtab")];
  ...
  // ---------- 事件绑定 ----------
  railRefs.refresh.addEventListener("click", () => loadAll());
  railRefs.newNovel.addEventListener("click", () => openCreateModal());
  railRefs.openFolder.addEventListener("click", () => openFromFolder());
  // Task 12：设置入口缺省分区为 model → 打开新「模型设置」页（模型分区已迁移）。
  railRefs.openSettings.addEventListener("click", () => openSettingsOrModelPage());
  railRefs.projectFilter?.addEventListener("input", () => renderProjectListFiltered());
  if (!drawerRefs.drawerClose.title) drawerRefs.drawerClose.title = "关闭抽屉";
  drawerRefs.drawerClose.addEventListener("click", () => closeDrawer());
  drawerRefs.drawerScrim.addEventListener("click", () => closeDrawer());
  drawerRefs.drawerTabs.addEventListener("click", (event) => {
  ...
  railRefs.openDrawer?.addEventListener("click", () => {
    closeTopbarSecondary();
    openDrawerTab("chapters");
  });
  
  // Round10：rail 覆盖态入口（窄屏 rail 不再 display:none）。
  railRefs.railToggle?.addEventListener("click", () => {
    if (railRefs.rail.classList.contains("show")) closeRail();
    else openRail();
  });
  railRefs.railScrim?.addEventListener("click", () => closeRail());
  ...
  function closeTopbarSecondary({ restoreFocus = false } = {}) {
    if (!railRefs.topbarSecondary?.classList.contains("show")) return;
    railRefs.topbarSecondary.classList.remove("show");
    railRefs.topbarMore?.setAttribute("aria-expanded", "false");
    if (restoreFocus) railRefs.topbarMore?.focus();
  }
  railRefs.topbarMore?.addEventListener("click", () => {
    const show = railRefs.topbarSecondary.classList.toggle("show");
    railRefs.topbarMore.setAttribute("aria-expanded", show ? "true" : "false");
  });
  document.addEventListener("click", (event) => {
    if (!railRefs.topbarSecondary?.classList.contains("show")) return;
    if (railRefs.topbarSecondary.contains(event.target) || railRefs.topbarMore.contains(event.target)) return;
    closeTopbarSecondary();
  });
  // Round10：菜单语义配套键盘——menuitem 聚焦时 ArrowDown/ArrowUp/Home/End 轮换。
  // 桌面行内常驻时同样生效（两枚按钮，行为无害且语义一致）。
  railRefs.topbarSecondary?.addEventListener("keydown", (event) => {
    if (!["ArrowDown", "ArrowUp", "Home", "End"].includes(event.key)) return;
    const items = [...railRefs.topbarSecondary.querySelectorAll('[role="menuitem"]')];
  ...
  function closeAppTopLayer() {
    if (settingsRefs.shortcutsScrim.classList.contains("show")) { closeShortcuts(); return true; }
    if (readerRefs.readerScrim.classList.contains("show")) { closeReader(); return true; }
    if (settingsRefs.settingsScrim.classList.contains("show")) { closeSettingsModal(); return true; }
    if (settingsRefs.createScrim.classList.contains("show")) { closeCreateModal(); return true; }
    if (drawerRefs.drawer.classList.contains("show")) { closeDrawer(); return true; }
    // rail 覆盖态优先级最低：顶层 modal/drawer 都未处理时才关闭。
    if (railRefs.rail.classList.contains("show")) { closeRail(); return true; }
    return false;
  }
  ...
  settingsRefs.createBrowse.addEventListener("click", () => browseForCreatePath());
  settingsRefs.createSubmit.addEventListener("click", () => initProject(settingsRefs.createPath.value.trim()));
  ...
  readerRefs.readerWide.addEventListener("click", () => {
    ...
    readerRefs.readerWide.setAttribute("aria-pressed", on ? "true" : "false");
  });
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if (event.key === "Escape") {
      ...
      if (railRefs.topbarSecondary.classList.contains("show")) {
    ...
    if (settingsRefs.shortcutsScrim.classList.contains("show")) focusTrap(settingsRefs.shortcutsScrim, event);
    else if (readerRefs.readerScrim.classList.contains("show")) focusTrap(readerRefs.readerScrim, event);
    else if (settingsRefs.settingsScrim.classList.contains("show")) focusTrap(settingsRefs.settingsScrim, event);
  ...
  function clearTransientState() {
    ...
    toaster.clearToast();
    if (readerRefs.readerScrim?.classList.contains("show")) {
      readerRefs.readerScrim.classList.remove("show");
      readerRefs.readerScrim.setAttribute("inert", "");
    }
  ...
  function refreshDrawerIfOpen() {
    if (!drawerRefs.drawer.classList.contains("show")) return;
    const top = drawerRefs.drawerBody.scrollTop;
    renderDrawerBody();
    drawerRefs.drawerBody.scrollTop = top;
  }
  ...
  async function browseForCreatePath() {
    ...
    const picked = await window.wwritingDesktop?.selectProjectFolder?.();
    if (picked) {
      settingsRefs.createPath.value = picked;
      setCreateStatus(`已选择：${picked}`, "info");
      settingsRefs.createTitle.focus();
  ...
  function resetCreateForm() {
    settingsRefs.createTitle.value = "";
    settingsRefs.createSeed.value = "";
    settingsRefs.createChapters.value = "100";
  ...
  function setCreateSubmitLoading(loading) {
    settingsRefs.createSubmit.disabled = loading;
    const spinner = settingsRefs.createSubmit.querySelector(".btn-spinner");
    const label = settingsRefs.createSubmit.querySelector(".btn-label");
  ...
  async function initProject(rawPath) {
    ...
    try {
      ...
      await postJson("/api/projects/init", {
        projectRoot,
        title: settingsRefs.createTitle.value.trim() || pathBaseName(projectRoot),
        story_seed: settingsRefs.createSeed.value.trim(),
        target_chapters: settingsRefs.createChapters.value,
  ...
  function setCreateStatus(text, kind) {
    settingsRefs.createStatus.textContent = text;
  ...
  function closeReader() {
    closeOverlay(readerRefs.readerScrim);
  }
  ...
  function renderCreateModalCopy() {
    ...
    else if (settingsRefs.createSubmit) settingsRefs.createSubmit.textContent = copy.submit;
  }
  ...
  function openCreateModal(prefillPath, options = {}) {
    ...
    setCreateStatus("", "");
    setCreateSubmitLoading(false);
    if (prefillPath) settingsRefs.createPath.value = prefillPath;
    renderCreateModalCopy();
    openOverlay(settingsRefs.createScrim, settingsRefs.createTitle);
    motion.openModal(settingsRefs.createScrim, document.querySelector("#create-card"));
  }
  
  function closeCreateModal() {
    settingsRefs.createScrim.dataset.closing = "true";
    settingsRefs.createScrim.classList.remove("show");
    settingsRefs.createScrim.setAttribute("inert", "");
    motion.closeModal(settingsRefs.createScrim, document.querySelector("#create-card"), {
      onComplete: () => {
        delete settingsRefs.createScrim.dataset.closing;
  ...
  function closeShortcuts() { closeOverlay(settingsRefs.shortcutsScrim); }
