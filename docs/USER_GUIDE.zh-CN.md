...
| 中央对话面 | **唯一的写作入口**：聊天、写作、审核、初始化都在这里发起；当前这一轮模型在做什么（思考中、读取文件、运行命令、等待确认）实时可见。 |
...
这是模型自主判断的范畴。模型可以在有明确字数要求时调用 `count_text` 获取真实字数，再自行决定补写或结束；应用不会因字数不足拒绝文件写入或提交，也不会显示"未达标"警告。错误只呈现一条事实，技术细节在活动行的折叠详情里。
