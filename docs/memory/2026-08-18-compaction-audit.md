...
**2026-08-23 订正**：第十三轮 resolveModelLimits 已字段化修复（model-identity.mjs，按模型查表），本条已过时。
...
- 发现 2（256k 一刀切）仍未修，前置条件不变：接入任何新 provider 前必须按模型查表。（2026-08-23 订正：见发现 2 注记，已字段化修复。）
