...
6. 两者成功后才调用 `applyLocalSecrets` 和刷新运行时配置。
...
```bash
git add src/core/settings-runtime.mjs src/core/app-server.mjs tests/settings-runtime.test.mjs tests/app-server-probe.test.mjs
...
3. 调用注入的 `complete`，提示词固定为要求仅返回 `OK`，`max_tokens` 不超过 4，超时 10 秒且不重试。
...
在测试导入中加入 `loadLocalSecretsSync`，并按现有测试模式用 `try/finally`
