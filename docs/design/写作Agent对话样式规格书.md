...
一个**只做一件事**的 Agent 对话界面：打开项目后，用户用自然语言下指令（"继续写第 3 章""帮我检查人物设定""/init"），Agent 自主决定读取哪些项目文件、运行哪些命令、修改哪些内容。**不存在独立的"开始写作"按钮、准备卡、章节完成卡或顶部运行面板**——写作通过聊天发起，章节阅读留在导航抽屉。
...
| # | 原则 | 说明 |
|---|------|------|
| P1 | 单一对话面 | 聊天、写作、审稿、蓝图初始化共用同一个输入框与对话流；斜杠菜单只负责补全输入，不建立第二套控制面 |
...
- 用户消息：右侧紧凑浅中性气泡；Agent 消息：左侧无框正文；两者都无头像无署名。
...
- 标题 `操作失败` + 一条用户可理解的事实（message），如 `本段 2380 字，低于 3200 字门槛。`。
...
- 补全项固定为 `/init`、`/write`、`/review`、`/model`、`/settings`。其中 `/settings`、`/model` 精确发送时只打开本地设置；另外三项仍作为普通消息交给统一 Agent。
...
- Agent 回复不使用卡片背景和边框；用户消息使用紧凑气泡。任务计划为右上悬浮层（唯一常规框选工具），活动记录使用横向分隔行，禁止把每条活动包成重复卡片。
- 空 Run、空活动列表和空队列必须完全隐藏，不能留下横线、占位条或状态装饰。
- 对话与 composer 共享 `--content-column: 900px`；窄视口（≤560px）排队行改双行网格，无重叠。
- 正文/输出区等宽字体 `var(--mono)`；UI 文字无衬线。
- Composer 是唯一常规浮动操作材料：8px 圆角、轻边界、克制阴影和半透明模糊；斜杠菜单从 composer 左下锚点出现。减少透明度时必须切换为实色表面。
