...
- Agent 工作过程（思考/工具/计划）以细边框圆角折叠卡片承载：收起一行状态+耗时，展开为明细；边框为发丝线（var(--agent-line)），禁止彩色边框与大面积底色；用户消息使用紧凑中性气泡；Assistant 正文保持无框直出，不包卡片。
- 活动记录以"整组一张卡片"承载；禁止每条活动行各包一张重复卡片。
