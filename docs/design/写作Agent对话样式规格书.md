...
> 第十轮关系：AgentSurface 的行为、状态机、事件与文案禁令继续以本文为准；全应用视觉、布局、排版和交互反馈以 `2026-08-15-round10-ui-maturity-spec.md` 为准，视觉冲突时由第十轮规格覆盖。
...
- 计划项为有序列表，每项 = 状态标记 + 步骤文本（可选描述）；标记为三态图标（completed 实心勾 + 步骤删除线 / in_progress 箭头圆 / pending 虚线圆）；面板头部带进度环（进行中 pie、全部完成实心勾）与滚动计数。
...
- composer 视觉对齐 ai-agent-input 外观语言（**提交在途**扫描边框、菜单信息卡、发送按钮在途态）；**不新增**附件/增强提示/技能 pill 功能。
...
- 引用：正文 `[^n]` 脚注语法渲染为行内上标（属正文排版）；提供来源数据时上标可点、消息末尾显示紧凑来源条（编号 + 标题 · 域名）；链接沿用本节安全规则（仅 http/https、`data-external-link` 委托）。
...
- 流式正文末尾显示实心光标（`assistant_message_delta` 期间实心常亮，`assistant_message_completed` 定稿后随流式气泡移除，无闪烁态）。
...
- “思考中”使用工作组思考项标签上的 shimmer 扫光（2.25s 三档渐变），思考项内容区只显示**最近 1–2 行推理片段**滚动替换（完整推理仍只在完成后经「思考 N 秒」项展开查看）；状态行保持状态点+文案。必须支持 `prefers-reduced-motion`（减弱后 shimmer 停、片段静态呈现）。
- **动效唯一性（2026-08-15 修订）**：并发动效上限 = 同时存在的**真实活动源数量**——思考项 shimmer、流式实心光标、web 搜索来源逐个 resolve 可并存；终态/用户折叠时必须立即移除工作组内的残余循环动效，终态 0 循环动效。
