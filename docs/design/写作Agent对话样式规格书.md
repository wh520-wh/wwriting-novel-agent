...
| # | 原则 | 说明 |
|---|------|------|
| P1 | 单一对话面 | 聊天、写作、审稿、蓝图初始化共用同一个输入框与对话流；不存在斜杠命令专属 UI |
...
| P10 | 折叠态即回看态 | Plan 与活动行使用原生 `<details>`；Run 终态后 Plan 折叠，展开可回看，无编辑入口 |
...
- 用户消息：右侧气泡；Agent 消息：左侧气泡；两者都无头像无署名。
...
- `<details>` 原生折叠，summary 为 `任务计划`。
- 计划项为有序列表，每项 = 状态标记 + 步骤文本；标记 `✓`（completed）/ `•`（in_progress）/ `○`（pending）。
- 活动 Run 时展开，Run 终态后折叠（同一节点只切换 open，不重建）；无任何编辑控件。
...
- placeholder：`输入消息`（无示例教学句）；发送按钮 `发送`；Enter 发送、Shift+Enter 换行。
...
- 底色/面板/文字沿用 `styles.css` 的 `--surface` / `--ink` / `--line` / `--muted` / `--faint` / `--accent` / `--red` 家族令牌，亮/暗自动翻转。
- 对话与 composer 共享 `--content-column: 900px`；窄视口（≤560px）排队行改双行网格，无重叠。
- 正文/输出区等宽字体 `var(--mono)`；UI 文字无衬线。
- 动效从简：状态变化不改变行高（按钮/标记固定最小尺寸），按钮尺寸不随状态抖动。
