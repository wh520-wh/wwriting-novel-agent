...
- Agent 工作过程以细边框圆角折叠卡片展示；用户消息保留紧凑中性气泡；Assistant 正文保持无框正文，不包卡片。
- 运行态/压缩态用紧凑状态条；禁止横贯对话区的全宽水平分隔线。
- 警告/错误只在确有语义时使用既有 amber/red 令牌。
- 验收矩阵不变：浅色/深色、窄窗口、prefers-reduced-motion，状态替换不引起布局跳动。
