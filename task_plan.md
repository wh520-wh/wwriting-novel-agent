...
> 当前产品收敛计划：2026-07-12
	> 状态：已执行并通过验收
> 说明：本节是当前阶段的产品优先级入口；下方历史计划保留作为技术和验收背景，不因本轮产品规划而删除。

## 当前阶段目标

在不引入明显新增维护面的前提下，把现有能力收敛成一条清晰的独立作者工作流：

```text
创建小说 → 配置并测试模型 → 输入故事种子 → 开始第 1 章
→ 阅读/审查/修订 → 继续下一章 → 导出成书
```

产品判断：当前代码的核心执行能力已经较完整，下一阶段重点不是增加 Agent、资料库或编辑器，而是把已有状态、记忆、审查、阅读和导出能力组织成用户可理解的工作台。

## 产品边界与非目标

- 保持单机、本地文件优先和独立作者定位。
- 优先复用现有 dashboard、agent_state、章节索引、chat、reviewer、memory、timeline、book-export 和模型配置能力。
- 不在本阶段新增云同步、账号、多人协作、插件市场、向量数据库、手机端、多智能体编排、完整人物/世界观编辑器、复杂时间线编辑器、EPUB 或自动更新系统。
- 不以增加面板数量作为完成标准；新增界面必须减少用户判断成本，或直接复用既有数据和 API。

## 阶段 0：发布与基础可信度（P0）

状态：待执行

目标：用户点击后确实得到预期结果，源码、打包应用和桌面快捷方式的行为一致。

任务：

1. 修复自定义模型预设的保存路径：空 `base_url` 时显示字段级错误，或在验证 fixture 中补齐合法地址；不得静默回退到旧模型。
2. 让 `verify:app-clickability` 通过，并恢复 `verify:local` 全绿。
3. 完成一次真实打包应用的人工点击验收，重点覆盖设置、新建小说、模型测试、开始写作、停止/重试、章节阅读和导出。
4. 发布前明确区分源码 HEAD、打包 commit、目录包 exe 和安装器版本，避免桌面快捷方式打开旧产物。

验收标准：

- 设置保存成功后，重启应用仍能看到完整模型配置和有效状态。
- 自定义模型保存失败时，用户能看到原因和具体字段，dashboard 不显示假成功。
- `npm test`、`npm run verify:app-shell`、`npm run verify:app-clickability`、`npm run verify:desktop-shell`、`npm run verify:local` 通过。
- 目录包和安装器启动的行为与开发运行时一致。

## 阶段 1：首次使用与”第一章成功”（P0）

状态：实施计划已完成，已执行并通过验收

设计文档：`docs/superpowers/specs/2026-07-12-first-chapter-success-design.md`

实施计划：`docs/superpowers/plans/2026-07-12-first-chapter-success.md`

目标：新用户无需理解内部模块，在一次连续路径内完成创建项目、配置模型并开始第一章。

任务：

1. 创建项目后进入“准备写作”状态，显示故事种子、章节目标、每章字数、当前模型和下一步主动作。
2. 首次运行前复用现有连接测试和配置校验，形成轻量预检：模型可用、项目可写、目标有效、联网权限满足时才允许开始。
3. 无模型时提供明确的“去配置并测试模型”；模型可用时提供“开始写第 1 章”。
4. 将空项目首页、聊天欢迎语和建议操作统一成一个下一步，不再让用户在多个面板间猜测。

验收标准：

- 新建项目后 3 个可见动作内能开始第一章。
- 缺模型、模型连接失败、项目不可写时，页面分别说明原因和解决动作。
- 点击“开始写第 1 章”后，能看到运行状态、章节阶段和可停止入口。
- 真实章节完成后，页面直接提供“阅读本章”和“继续下一章”。

## 阶段 2：模型库与项目选择（P0）

状态：待执行

目标：模型配置成为本机可复用资源，项目只选择当前使用的模型，避免重复填写和配置漂移。

任务：

1. 复用 `local-model-profiles` 和本机 secrets，把供应商、模型、连接状态和 API Key 保存归入应用级模型库。
2. 新建或打开项目时直接选择已有模型；项目只保存当前模型引用或快照，不要求重新输入 key。
3. 设置页显示“可用 / 未测试 / 连接失败”状态，并保留现有测试连接能力。
4. 项目顶栏或准备写作卡显示当前模型名称；切换模型后明确提示影响范围。

验收标准：

- 保存一个模型后，新建第二个项目可以直接选择该模型。
- 重启应用后模型列表、选中模型和连接状态可恢复。
- 项目配置不保存明文 key；运行时仍能从本机 secrets 解析真实 key。
- 模型切换不会修改小说正文、章节索引或写作记忆。

## 阶段 3：信息架构收敛（P1）

状态：待执行

目标：保留底层模块，但将作者看到的主任务收敛为“写作 / 故事资料 / 项目管理”。

任务：

1. “写作”承载当前章节、Agent 对话、运行进度、停止/继续/重试和确认卡。
2. “故事资料”承载章节摘要、人物/设定记忆、连续性信息、故事时间线和外部资料。
3. “项目管理”承载章节列表、审查结果、成本、导出、归档和诊断。
4. 技术指标、事件日志、缓存和权限仍可查看，但作为二级详情，不抢占首次使用的主视线。
5. 统一项目状态和主动作：运行中显示“停止”，等待确认显示“查看并确认”，已暂停显示“继续”，需处理显示“查看原因”，已完成显示“阅读本章”。

验收标准：

- 用户不需要先理解“运行、成本、技能、审查”之间的技术关系，仍能完成写作、阅读和继续写作。
- 所有现有能力仍可访问，未因导航收敛而删除 API 或底层数据。
- 任一项目状态在顶栏、写作区和章节卡中的文案一致。
- 窄屏和桌面窗口下主要动作仍可点击，相关三项 clickability/shell 验证保持通过。

## 阶段 4：章节生成后的编辑闭环（P1）

状态：待执行

目标：生成不是终点，用户能立即阅读、理解问题、修订并继续下一章。

任务：

1. 复用章节索引、质量门禁、审查报告、diff 和阅读器，形成章节完成卡。
2. 完成卡显示真实字数、审查状态、连续性提醒和下一步动作。
3. 审查问题使用“问题 → 证据 → 动作”的语言，优先复用现有对话确认和修订工具。
4. 继续保留“阅读本章、查看问题、让智能体修正、继续下一章”等明确动作，不新增独立编辑器。

验收标准：

- 章节完成后用户能一键打开干净正文。
- 审查失败时能定位问题来源，并能通过确认卡发起修订。
- 修订前后可查看段落差异，正式章节文件仍由文件工具落盘。
- 用户确认忽略问题后，系统保留审查记录，不伪造“全部通过”。

## 阶段 5：故事记忆可见化（P1）

状态：待执行

目标：把现有记忆和时间线能力变成用户能理解的“这本书目前记住了什么”。

任务：

1. 用轻量只读区域展示主要人物、最近章节摘要、未收束伏笔、当前故事时间和连续性提醒。
2. 所有内容来自现有 memory、chapter index、continuity 和 timeline artifact，不新建一套独立编辑数据模型。
3. 用户需要修改时，继续通过对话提出变更，经现有确认/写作通道处理。

验收标准：

- 打开项目后能看到最近章节和当前故事状态，不必阅读日志文件。
- 故事记忆与章节文件、事件日志之间出现冲突时能显示“需要核对”，不静默覆盖。
- 旁路询问仍保持只读，不启动或污染正式写作任务。

## 阶段 6：成书交付（P1）

状态：待执行

目标：用户最终得到可定位、可复用的成书文件，而不是只得到项目目录和日志。

任务：

1. 复用 `book-export` 和现有导出工具，将“导出成书”提升为项目管理中的固定动作。
2. 导出前显示已完成章节、缺失章节和审查提醒。
3. 支持导出全部章节或指定范围，完成后显示真实文件路径并提供打开文件夹。
4. 记录最近一次导出结果，但不引入云端发布或复杂格式转换。

验收标准：

- 用户可以从项目主界面完成一次导出，不必打开文件系统寻找脚本产物。
- 缺失章节和审查问题有提示，但不会无理由阻止用户导出。
- 导出的 Markdown/TXT 内容与章节文件顺序、标题和正文一致。
- 导出失败时保留原章节文件，并给出可操作的错误信息。

## 阶段 7：发布前总验收

状态：待执行

目标：把产品改进变成可发布版本，而不是只在开发环境中看起来可用。

任务：

1. 每个阶段补充对应的单元、服务端、app-shell 和真实 Electron clickability 验收。
2. 每轮 UI 或 Electron 改动后执行项目要求的四项验证，交付前额外执行 `verify:local`。
3. 重新打包目录包和安装器，确认桌面快捷方式指向最新 exe。
4. 记录未执行的真实 API 验收和人工桌面验收，不把代码覆盖等同于用户验收。

最终完成标准：

- 第一章成功路径完整可用。
- 模型可跨项目复用且重启后可确认。
- 生成、阅读、审查、修订、继续写作形成闭环。
- 记忆、时间线、导出能力可被用户发现和使用。
- 本地自动化验证和打包应用验收均通过。
