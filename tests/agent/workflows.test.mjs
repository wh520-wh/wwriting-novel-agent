...
//     commit_chapter（统一目录由 runtime 固定提供，不随请求变化）
...
// 统一深工具目录（固定三件套，Task 7 契约）
const UNIFIED_DEEP_NAMES = ["append_chapter_segment", "commit_chapter", "update_plan"];
...
// ---------------------------------------------------------------------------
// 统一目录：每轮同一工具集，深工具恒为三件套
// ---------------------------------------------------------------------------

test("每轮工具目录相同：深工具恒为 update_plan/append_chapter_segment/commit_chapter", async (t) => {
  const h = await createProjectAgentHarness({
    gatewayScript: [
      async (request) => {
        ...
        assert.deepEqual(
          ...
          "深工具目录必须恒为 update_plan/append_chapter_segment/commit_chapter"
        );
