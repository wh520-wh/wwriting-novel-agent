...
import { buildPricingTable, fillDeepSeekPricing, isDeepSeekModel, normalizePricing, resolvePricing } from "../src/core/model-pricing.mjs";
...
test("fillDeepSeekPricing 为 v4-flash 补全官方人民币价（1.00/2.00/0.02）", () => {
  const filled = fillDeepSeekPricing("deepseek-v4-flash", "https://api.deepseek.com", {});
...
test("fillDeepSeekPricing 为 v4-pro 补全官方人民币价（3.00/6.00/0.025）", () => {
  const filled = fillDeepSeekPricing("deepseek-v4-pro", "https://api.deepseek.com", {});
...
test("fillDeepSeekPricing 只补缺失字段，用户已填的值绝不覆盖", () => {
  const filled = fillDeepSeekPricing("deepseek-v4-flash", "https://api.deepseek.com", { input_per_million: 0.5, cache_hit_per_million: 0.123 });
  assert.equal(filled.input_per_million, 0.5);
  assert.equal(filled.output_per_million, 2.0);
  assert.equal(filled.cache_hit_per_million, 0.123);
});

test("fillDeepSeekPricing 未收录的 deepseek 模型不补填（交给 ×2% 默认折算）", () => {
  const filled = fillDeepSeekPricing("deepseek-chat", "https://api.deepseek.com", { input_per_million: 1, output_per_million: 2 });
  assert.equal(filled.cache_hit_per_million, undefined);
});

test("fillDeepSeekPricing 非 DeepSeek 模型不补填", () => {
  const filled = fillDeepSeekPricing("mimo-v2.5-pro", "https://api.xiaomimimo.com/v1", { input_per_million: 1, output_per_million: 2 });
  assert.equal(filled.cache_hit_per_million, undefined);
});
